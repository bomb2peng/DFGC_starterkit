...
def load_pretrained_weights(model, model_name, load_fc=True, advprop=False):  ## note,load_fa default is True
    """ Loads pretrained weights, and downloads if loading for the first time. """
    # AutoAugment or Advprop (different preprocessing)
    url_map_ = url_map_advprop if advprop else url_map
    state_dict = torch.load("/home/fanhongxing/model/efficientnet-b3-c8376fa2.pth",map_location='cpu')
    # state_dict = torch.load("/hd1/fanhongxing/DFDC/out/202003276/0_1999_efficient.ckpt",map_location='cpu')
    # state_dict = model_zoo.load_url(url_map_[model_name])
    if load_fc:
        # model._fc = nn.Linear(1536, 1)  #fhx add
